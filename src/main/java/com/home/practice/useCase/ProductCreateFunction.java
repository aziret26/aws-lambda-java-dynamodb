...
import com.home.practice.service.PropertyStorage;
import com.home.practice.service.RequestService;
import com.home.practice.service.ResponseService;
 ...
 */
public class ProductCreateFunction implements RequestHandler<AwsProxyRequest, ProductsProxyResponse> {
    ...
    private static final Logger LOG = LogManager.getLogger(ProductCreateFunction.class);
    ...
    private ProductsProxyResponse create(AwsProxyRequest request) {
        // Parsing an input request
        AddProductRequest productRequest = requestService.getAddProductRequest(request);

        // Saving request to DB
        Product product = dbService.putProductRequest(productRequest);
        LOG.info("ProductRequest has been written to DB.");

        return responseService.buildResponse(200,
                String.format("Message %s has been sent successfully.", product.getId()));
    }
